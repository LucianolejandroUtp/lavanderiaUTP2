...
public class DetalleComprobanteJpaController implements Serializable {
  ...
  public void create(DetalleComprobante detalleComprobante) {
    ...
    try {
      ...
      if (comprobanteId != null) {
        comprobanteId.getDetalleComprobanteCollection().add(detalleComprobante);
        comprobanteId = em.merge(comprobanteId);
      }
      if (servicioId != null) {
        servicioId.getDetalleComprobanteCollection().add(detalleComprobante);
  ...
  public void destroy(Long id) throws NonexistentEntityException {
    ...
    try {
      ...
      Comprobante comprobanteId = detalleComprobante.getComprobanteId();
      if (comprobanteId != null) {
        comprobanteId.getDetalleComprobanteCollection().remove(detalleComprobante);
        comprobanteId = em.merge(comprobanteId);
      }
      Servicio servicioId = detalleComprobante.getServicioId();
      if (servicioId != null) {
        servicioId.getDetalleComprobanteCollection().remove(detalleComprobante);
        servicioId = em.merge(servicioId);
      }
